...
# 3.编写描述符MyDes，使用文件来存储属性，属性的值会直接存储到对应的pickle的文件中。如果属性被删除了，文件也会同时被删除，属性的名字也会被注销。
...
class MyDes:
    ...
    def __init__(self, name = 'x'):
        self.name = name
        file = open('/Users/yasmine/Documents/学习/language/python/pickle.txt', 'a')
        file.close()
            
    def __set__(self, instance, value):
        self.value = value
        file = open('/Users/yasmine/Documents/学习/language/python/pickle.txt', 'a')
        file.write('The %s = %d'%(self.name, self.value))
        file.close()
